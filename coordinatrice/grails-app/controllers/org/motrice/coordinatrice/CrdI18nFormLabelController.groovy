...
class CrdI18nFormLabelController {
   ...
   * id must be defined and must be the form definition path
   */
  def listkey(Integer max) {
    ...
    def key = params.id
    if (key) {
      def instSet = new TreeSet(CrdI18nFormLabel.findAllByFormdefPath(key, params))
      def instTotal = CrdI18nFormLabel.countByFormdefPath(key)
    } else {
      redirect(action: "list")
    }
    [formdefKey: key, formLabelInstList: instSet, formLabelInstTotal: instTotal]
  }
