...
public class SearchProcessInstancesStartedByUser extends ServerResource {
	...
	public static final Logger log = LoggerFactory.getLogger(SearchProcessInstancesStartedByUser.class.getName());
	...
	public PagedProcessInstanceSearchResult searchProcessInstancesStartedByUser() {
		...
		log.info("REST searchProcessInstancesStartedByUser with parameter searchForUserId=[" + searchForUserId + "] fromIndex=[" + fromIndex + "] pageSize=[" + pageSize + "] sortBy=["  + sortBy + "] sortOrder=[" + sortOrder + "] filter=[" + filter + "] userId=[" + userId + "]");
