...
FORM_BUILDER_JAR="orbeon-form-builder.jar"
FORM_BUILDER_JAR_ORIG="$LIB/$FORM_BUILDER_JAR"
SWEDISH_JAR="orbeon-builder-4.6-lang-sv-added.tgz"
...
if [ ! -f "$FORM_BUILDER_JAR_ORIG" ]; then
    echo "Could not find $FORM_BUILDER_JAR_ORIG"
    exit 1
fi
...
cp "$FORM_BUILDER_JAR_ORIG" "$ORIG_DIR"
cp "$FORM_BUILDER_JAR_ORIG" "$TARGET_DIR"

# Unpack the source jar
( cd "$TMP_DIR" ; tar zxf "$SWEDISH_JAR" )

# Replacement in orbeon-form-builder.jar
echo "--- Replacement in `basename $FORM_BUILDER_JAR`"
( cd "$TARGET_DIR" ; \
  jar ufv "$FORM_BUILDER_JAR" -C "$TMP_DIR" forms )

# Copy the updated jars back to their original location
# A backup copy is made to the 'orig' subdirectory
cp "$TARGET_DIR"/orbeon-form-builder.jar "$FORM_BUILDER_JAR_ORIG"
