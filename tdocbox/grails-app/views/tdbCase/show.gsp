	...
	<body>
		...
		<div id="show-tdbCase" class="content scaffold-show" role="main">
			...
			<ol class="property-list tdbCase">
				...
				<g:if test="${tdbCaseObj?.drill}">
				<li class="fieldcontain">
					<span id="drill-label" class="property-label"><g:message code="tdbCase.drill.label" default="Drill" /></span>
					
						<span class="property-value" aria-labelledby="drill-label"><g:link controller="tdbDrill" action="show" id="${tdbCaseObj?.drill?.id}">${tdbCaseObj?.drill?.encodeAsHTML()}</g:link></span>
