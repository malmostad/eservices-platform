...
class PackageService {
   ...
   * Return List of OriFormdef (not sorted).
   */
   ...
   */
  OriFormdef singleLocalFormdef(id) {
   ...
   * Return List of OriFormdefVer (not sorted).
   */
  List allLocalFormdefVers(OriFormdef formdef) {
   ...
   * Return List of OriItem.
   */
  List allLocalDefItems(OriFormdef formdef) {
   ...
   */
  OriPackage createExportPackage(String packageName, params) {
    if (log.debugEnabled) log.debug "createExportPackage << ${packageName} ..."
    def siteName = grailsApplication.config.orifice.local.site.name
    def pkg = new OriPackage(siteName: siteName, packageName: packageName,
    packageFormat: packageFormat(), siteTstamp: new Date(), originLocal: true)
    if (!pkg.save(insert: true)) log.error "OriPackage save: ${pkg.errors.allErrors.join(',')}"
    ...
    formdefList.each {formdef ->
      ...
      if (!formdef.save(insert: true)) log.error "OriFormdef save: ${formdef.errors.allErrors.join(',')}"
	...
	if (!fdv.save(insert: true)) log.error "OriFormdefVer save: ${fdv.errors.allErrors.join(',')}"
      }
	...
	pkg.addToItems(item)
	if (!item.save(insert: true)) log.error "OriItem save: ${item.errors.allErrors.join(',')}"
      }
    }
  ...
  private URL localPostxdb(String tail) {
    def head = grailsApplication.config.orifice.postxdb.uri
