...
<div class="fieldcontain ${hasErrors(bean: migItemInst, field: 'pkg', 'error')} required">
	<label for="pkg">
		<g:message code="migItem.pkg.label" default="Pkg" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="pkg" name="pkg.id" from="${org.motrice.migratrice.MigPackage.list()}" optionKey="id" required="" value="${migItemInst?.pkg?.id}" class="many-to-one"/>
</div>
