(function() {
  $(function() {
    var Builder, ControlSelector, Events, LabelHintSelector, OD, annotateWithLhhaClass, clickOrFocus, currentControl, currentLabelHint, endEdit, htmlClass, isLabelHintHtml, labelHintEditor, labelHintValue, lhha, setLabelHintHtml, startEdit;
    ...
    lhha = function() {
      if (currentLabelHint.is('.xforms-label')) {
        return 'label';
      } else {
        return 'hint';
      }
    ...
    isLabelHintHtml = function() {
      return currentControl.is('.' + htmlClass());
    };
    setLabelHintHtml = function(isHtml) {
      return currentControl.toggleClass(htmlClass(), isHtml);
    };
    ...
    labelHintValue = function(value) {
      var valueAccessor;
      valueAccessor = isLabelHintHtml() ? currentLabelHint.html : currentLabelHint.text;
    ...
    startEdit = function() {
      ...
      labelHintEditor().checkbox.tooltip({
        title: $('.fb-lhha-checkbox-message').text()
      });
    ...
    $('.fb-main').on('click', LabelHintSelector, function(_arg) {
      ...
      if (!_.isNull(currentControl)) {
        endEdit();
      }
      currentLabelHint = $(currentTarget);
      th = currentLabelHint.parents('th');
      currentControl = th.is('*') ? (trWithControls = th.parents('table').find('tbody tr.fb-grid-tr').first(), tdWithControl = trWithControls.children(':nth-child(' + (th.index() + 1) + ')'), tdWithControl.find(ControlSelector)) : currentLabelHint.parents(ControlSelector).first();
      return startEdit();
    });
    Builder.controlAdded.add(function(containerId) {
      var container, repeat;
      container = $(document.getElementById(containerId));
      currentControl = container.find(ControlSelector);
      repeat = container.parents('.fr-repeat').first();
      currentLabelHint = repeat.is('*') ? repeat.find('thead tr th:nth-child(' + (container.index() + 1) + ') .xforms-label') : container.find('.xforms-label');
      return startEdit();
    });
    return (function() {
      var gridTdUpdated, lhhaNames, placeholderText, updateGridTd, updatePlacehodlerText;
      lhhaNames = ['label', 'hint'];
      gridTdUpdated = [];
      placeholderText = {};
      updateGridTd = function(gridTd) {
        gridTd = $(gridTd);
        return _.each(lhhaNames, function(lhha) {
          var elementInDiv;
          elementInDiv = gridTd.find(".xforms-" + lhha);
          return elementInDiv.attr('placeholder', placeholderText[lhha]);
        });
      };
      updatePlacehodlerText = function() {
        var foundDifference, isInDoc;
        foundDifference = false;
        _.each(lhhaNames, function(lhha) {
          var newText;
