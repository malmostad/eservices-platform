			...
			<div class="tips">
				<div>
					<div id="svid12_1d8b012512b3897f473800038729">
						...
						<p>
							<a href="#" id="add-comment">Kommentera √§rendet</a>
						</p>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function refreshTags(info) {
		console.log("refreshTags TODO, asynkront anrop l√§gger till taggar s√• t√§nk ut vettigaste s√§ttet att √•terkoppla");
	}
	...
	function refreshActivityWorkflowInfo(info) {
		...
		if (!$.isEmptyObject(info) && !$.isEmptyObject(info.assignedUser) && !$.isEmptyObject(info.assignedUser.uuid)) {
			...
			if (info.assignedUser.uuid == '${user.uuid}') {
				/* Assigned to me */
				str = "Jag ‰r tilldelad att utfˆra aktiviteten";
				$("#assign-to-me").hide();
				$("#unassign").show();
				...
				str = info.assignedUser.label
						+ " ‰r tilldelad att utfˆra aktiviteten";
				$("#assign-to-me").show();
				$("#unassign").show();
				$("#edit-candidates").hide();
			}
			...
			if ($.isEmptyObject(info) || $.isEmptyObject(info.candidates)) {
				str = "Ingen kandidat till att utfˆra aktiviteten";
			} else {
				...
				str += " ‰r kandidater till att utfˆra aktiviteten";
			}
			$("#assign-to-me").show();
			$("#unassign").hide();
