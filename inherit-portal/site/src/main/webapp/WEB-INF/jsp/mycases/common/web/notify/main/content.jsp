...
<c:choose>
  ...
  <c:otherwise>
	...
	<c:if test="${not empty activity}">
    	<script type="text/javascript" charset="utf-8">
		        ...
		        $j(document).ready(function () {
		             $j("#xform").load("<fmt:message key="orbeonbase.portal.url"/>${activity.formUrl}orbeon-embeddable=true", function(data) {
	...
	<div class="row-fluid">
		<div class="span12">
    		<div id="xform" class="public-xform">Laddar delgivning...</div>
    		
    		<form method="post" action="noform/confirm">
						<input type="hidden" name="document" value="${activity.formDocId}" />
						<input type="submit" value="Ok! Jag har l&auml;st"/>
			</form>
		</div>
	</div>    
